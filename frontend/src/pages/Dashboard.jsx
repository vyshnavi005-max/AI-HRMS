...
function DonutRing({ value, total, color, label }) {
    ...
    return (
        <div className="flex flex-col items-center gap-1">
            ...
            <span className="text-stone-900 font-bold text-lg -mt-1">{value}</span>
...
export default function Dashboard() {
    ...
    if (loading) return (
        <div className="flex items-center justify-center py-20">
            <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-stone-400 text-sm font-medium">Loading dashboard...</p>
            </div>
        </div>
    );
    ...
    return (
        <>
            <div className="mb-6">
                <h1 className="text-xl lg:text-2xl font-bold text-stone-900">Workforce Dashboard</h1>
                <p className="text-stone-500 text-sm mt-0.5">Real-time overview of your organization</p>
            </div>

            {/* Stat Cards */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
                {[
                    { label: 'Total Employees', value: emp.total || 0, sub: `${emp.inactive || 0} inactive`, Icon: LuUsers, bg: 'bg-orange-50', border: 'border-orange-100', val: 'text-orange-700', icon: 'text-orange-400' },
                    ...
                    <div key={label} className={`${bg} border ${border} rounded-2xl p-4`}>
                        ...
                        <p className="text-stone-700 text-xs font-semibold">{label}</p>
            ...
            <div className="flex flex-col lg:grid lg:grid-cols-3 gap-4 mb-6">
                <div className="lg:col-span-2 bg-white border border-stone-200 rounded-2xl p-5 shadow-sm">
                    <h2 className="text-stone-900 font-semibold text-sm mb-4">Task Status Overview</h2>
                    <div className="flex items-center justify-around">
                        <DonutRing value={Number(tasks.assigned) || 0} total={Number(tasks.total) || 1} color="#ea580c" label="Assigned" />
                    ...
                    <div className="mt-4 pt-4 border-t border-stone-100 flex justify-between text-xs text-stone-400">
                        <span>Total: <span className="text-stone-700 font-semibold">{tasks.total || 0}</span></span>
                ...
                <div className="bg-white border border-stone-200 rounded-2xl p-5 shadow-sm">
                    <h2 className="text-stone-900 font-semibold text-sm mb-4">Employees by Dept</h2>
                    {depts.length === 0 ? (
                        <p className="text-stone-400 text-xs">No department data yet.</p>
                    ) : (
                                ...
                                return (
                                    <div key={d.department}>
                                        <div className="flex justify-between text-xs mb-1">
                                            <span className="text-stone-600 truncate">{d.department}</span>
                                            <span className="text-stone-400 ml-2 shrink-0">{d.count}</span>
                                        </div>
            ...
            {/* Bottom row */}
            <div className="flex flex-col lg:grid lg:grid-cols-2 gap-4 mb-8">
                <div className="bg-white border border-stone-200 rounded-2xl p-5 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                            <LuBrain size={15} className="text-orange-500" />
                            <h2 className="text-stone-900 font-semibold text-sm">AI Productivity Scores</h2>
                        </div>
                            ...
                            {top.map((e, i) => (
                                <div key={e.id}>
                                    <div className="flex items-center gap-3 mb-1.5">
                                        <span className="text-xs text-stone-400 w-3">{i + 1}</span>
                                        <div className="w-7 h-7 rounded-lg bg-orange-100 flex items-center justify-center text-orange-600 font-bold text-xs shrink-0">
                                            {e.name[0]}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-stone-800 text-xs font-medium truncate">{e.name}</p>
                                            <p className="text-stone-400 text-[10px]">{e.role} · {e.completed_tasks}/{e.total_tasks} tasks</p>
                                        </div>
                                    </div>
                                    <div className="ml-10">
                ...
                <div className="bg-white border border-stone-200 rounded-2xl p-5 shadow-sm">
                    <div className="flex items-center gap-2 mb-4">
                        <LuClock size={15} className="text-stone-400" />
                        <h2 className="text-stone-900 font-semibold text-sm">Recent Tasks</h2>
                    </div>
                    {recent.length === 0 ? (
                        ...
                        <div className="space-y-3">
                            {recent.map((task) => (
                                <div key={task.id} className="flex items-start gap-3">
                                    <span className={`mt-1.5 w-1.5 h-1.5 rounded-full shrink-0 ${PRIORITY_DOT[task.priority] || 'bg-stone-300'}`} />
                                    <div className="flex-1 min-w-0">
                                        <p className="text-stone-800 text-xs font-medium truncate">{task.title}</p>
