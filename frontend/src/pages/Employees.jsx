    ...
    return (
        <>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-3">
                <div>
                    <h1 className="text-xl lg:text-2xl font-bold text-stone-900">Employees</h1>
                    <p className="text-stone-500 text-sm mt-0.5">{employees.length} total members</p>
                </div>
                <button onClick={openAdd} className="bg-orange-600 hover:bg-orange-700 text-white font-semibold px-4 py-2.5 rounded-xl text-sm transition-all shadow-sm flex items-center gap-2 self-start sm:self-auto">
            ...
            <div className="mb-5">
                <div className="relative">
                    <LuSearch size={14} className="absolute left-3 top-1/2 -transtone-y-1/2 text-stone-400" />
                    <input type="text" placeholder="Search employees..." value={search} onChange={(e) => setSearch(e.target.value)}
                        className="w-full bg-stone-50 border border-stone-200 text-stone-900 placeholder-stone-400 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100 transition pl-9" />
                </div>
            </div>
            ...
            <div className="lg:hidden space-y-3">
                ...
                ) : filtered.length === 0 ? <div className="text-center text-stone-400 py-16">{search ? 'No results.' : 'No employees yet.'}</div>
                    : filtered.map((emp) => (
                        <div key={emp.id} className="bg-white border border-stone-200 rounded-2xl p-4 sm:p-5 shadow-sm transition-all duration-200 hover:border-orange-200 hover:shadow-md">
                            <div className="flex items-start gap-3">
                                <div className="w-10 h-10 rounded-xl bg-orange-100 flex items-center justify-center text-orange-600 font-bold text-sm shrink-0 shadow-sm">
                                    {emp.name[0].toUpperCase()}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between gap-2 mb-1">
                                        <p className="font-semibold text-stone-900 text-sm truncate">{emp.name}</p>
                                        <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold border ${emp.is_active ? 'bg-green-50 text-green-600 border-green-100' : 'bg-red-50 text-red-600 border-red-100'}`}>
                                            {emp.is_active ? 'Active' : 'Inactive'}
                                        </span>
                                    </div>
                                    <p className="text-xs text-stone-400 mb-2 truncate">{emp.role} · {emp.department}</p>
                                    {(emp.skills || []).length > 0 && (
                                        <div className="flex flex-wrap gap-1 mt-2">
                                            {emp.skills.slice(0, 3).map((s) => (
                                                <span key={s} className="bg-orange-50 text-orange-600 text-[10px] px-1.5 py-0.5 rounded border border-orange-100">{s}</span>
                                            ))}
                                            {emp.skills.length > 3 && <span className="text-stone-400 text-[10px]">+{emp.skills.length - 3}</span>}
                                        </div>
                                    )}
                                    <div className="flex gap-2 mt-3">
                                        <button onClick={() => openEdit(emp)} className="flex items-center gap-1 text-xs bg-stone-100 hover:bg-stone-200 text-stone-700 px-3 py-1.5 rounded-lg transition"><LuPencil size={11} /> Edit</button>
            ...
            {/* Desktop table */}
            <div className="hidden lg:block bg-white border border-stone-200 rounded-2xl overflow-hidden shadow-sm">
                {loading ? (
                    <div className="flex flex-col items-center gap-3 py-16">
                        <div className="w-10 h-10 rounded-full border-4 border-orange-100 border-t-orange-600 animate-spin" />
                        <p className="text-stone-400 text-sm">Loading employees...</p>
                    </div>
                ) : filtered.length === 0 ? <div className="text-center text-stone-400 py-16">{search ? 'No employees match your search.' : 'No employees yet. Add your first employee!'}</div>
                    : (
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-stone-100 bg-stone-50">
                                    {['Employee', 'Role', 'Department', 'Skills', 'Status', 'Tasks', ''].map((h) => (
                                        <th key={h} className="text-left text-xs font-semibold text-stone-400 uppercase tracking-wider px-6 py-4">{h}</th>
                                    ))}
                                </tr>
                            </thead>
                                    ...
                                    <tr key={emp.id} className="hover:bg-stone-50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-9 h-9 rounded-xl bg-orange-100 flex items-center justify-center text-orange-600 font-bold text-sm">{emp.name[0].toUpperCase()}</div>
                                                <div>
                                                    <p className="text-stone-900 font-medium text-sm">{emp.name}</p>
                                                    <p className="text-stone-400 text-xs">{emp.email}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-stone-700 text-sm">{emp.role}</td>
                                        <td className="px-6 py-4 text-stone-700 text-sm">{emp.department}</td>
                                        <td className="px-6 py-4">
                                            <div className="flex flex-wrap gap-1">
                                                {(emp.skills || []).slice(0, 3).map((skill) => (
                                                    <span key={skill} className="bg-orange-50 text-orange-600 text-xs px-2 py-0.5 rounded-md border border-orange-100">{skill}</span>
                                                ))}
                                                {(emp.skills || []).length > 3 && <span className="text-stone-400 text-xs">+{emp.skills.length - 3}</span>}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full ${emp.is_active ? 'bg-green-50 text-green-600 border border-green-100' : 'bg-stone-100 text-stone-500 border border-stone-200'}`}>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                                                <button onClick={() => openEdit(emp)} className="flex items-center gap-1 text-xs bg-stone-100 hover:bg-stone-200 text-stone-700 px-2.5 py-1.5 rounded-lg transition"><LuPencil size={12} /> Edit</button>
                ...
                <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white border border-stone-200 rounded-2xl w-full max-w-lg p-6 shadow-2xl shadow-stone-200 max-h-[90vh] overflow-y-auto">
                        <div className="flex items-center justify-between mb-5">
                            <h2 className="text-lg font-semibold text-stone-900">{editEmployee ? 'Edit Employee' : 'Add New Employee'}</h2>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-xs font-medium text-stone-600 mb-1.5">Department *</label>
                            ...
                            <div>
                                <label className="block text-xs font-medium text-stone-600 mb-1.5">Skills <span className="text-stone-400">(comma-separated)</span></label>
                                <input name="skills" value={form.skills} onChange={handleChange} placeholder="React, Node.js, PostgreSQL" className={fieldCls} />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-stone-600 mb-1.5">Login Password <span className="text-stone-400">{editEmployee ? '(leave blank to keep current)' : '(for employee portal access)'}</span></label>
                            ...
                            <div>
                                <label className="block text-xs font-medium text-stone-600 mb-1.5">Wallet Address <span className="text-stone-400">(optional)</span></label>
                                <input name="wallet_address" value={form.wallet_address} onChange={handleChange} placeholder="0x..." className={fieldCls} />
                            </div>
                            {editEmployee && (
                                <div className="flex items-center gap-2">
                                    <input type="checkbox" name="is_active" id="is_active" checked={form.is_active} onChange={handleChange} className="w-4 h-4 accent-orange-600" />
                                    <label htmlFor="is_active" className="text-sm text-stone-700">Active employee</label>
                                </div>
                            )}
